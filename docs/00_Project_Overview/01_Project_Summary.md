...
1. Build the network with OPNsense, a managed Omada switch, VLAN-mapped wireless SSIDs, and the Omada Controller on Management VLAN 10.
...
| Omada Controller container, VLAN 10 | Separate NucBox management container for switch/AP management, restricted admin access, configuration backup and recovery |
| NucBox hypervisor | Intel Core i9-13900HK, 32 GB DDR5 RAM, 1 TB SSD; VLAN-aware bridge, four isolated application VMs, management on VLAN 10 |
...
Docker Compose is planned inside the application VMs. The Omada Controller belongs in a separate NucBox management container on VLAN 10; its resource allocation still needs to be validated against the selected hardware. It is not assigned to a public or personal-data VM. Attack Lab test VMs are temporary resources outside the four persistent NucBox VMs.

## Current boundary

This repository is planning documentation. The revised four-VM design and migration are not deployed or validated by this update. The KEV dashboard already has a separate live Ubuntu deployment; its move to the NucBox must follow a verified cutover. The owner-provided CPU, RAM, and SSD specifications are recorded above; the exact SKU, available resources, virtualization support, and per-workload allocations still need validation.
