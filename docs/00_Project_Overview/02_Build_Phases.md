...
- Record the exact NucBox SKU and verify the stated Intel Core i9-13900HK, 32 GB DDR5 RAM, 1 TB SSD, virtualization support, usable storage, backup capacity, and headroom for Wazuh indexing, Jellyfin transcoding, and the Omada Controller container.
...
- Configure Omada switch Port 3 as the NucBox trunk for VLANs 10, 30, 70, 80, and 81; map SSIDs and access ports. Reserve a Management VLAN address for the Omada Controller container.
...
- Deploy the Omada Controller in a dedicated NucBox management container on VLAN 10 after validating resource headroom; restrict its UI to approved admin/VPN paths. Back up switch/AP settings, adopt one device at a time, and revalidate trunks, SSIDs, DHCP, and management reachability.
...
- Verify VPN-only paths for Nextcloud and Jellyfin and management-only paths for Wazuh, infrastructure, and the Omada Controller. Test controller backup/restore and device adoption recovery.
